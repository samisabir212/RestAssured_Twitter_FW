...
public class Twitter_API_Play {
	...
	public void setup() {
		...
		responseSpecification = RestUtilities.getResponseSpecification(); //now you have a responseSpec object to use in TC
	...
	public void readtweet() {
		...
		.then()
			...
			.body("user.name", matcher.hasItem("sami sabir"));
	...
	public void readTweet_withQuery_Param_limit_Count_Test() {
		 ...
		 */
		given()
		.spec(RestUtilities.createQueryParam(requestSpecification, "count", "2"))
	.when()
		.get(EndPoints.STATUSES_USER_TIMELINE) //reading tweet from time line not read show tweet
	.then()
		.log() //logs output to console
		.body()
		.spec(responseSpecification)
		.body("user.name", matcher.hasItem("sami sabir"));
	...
	public void verifyResponseTweetExists() {
		...
		Response response = RestUtilities.getResponse(RestUtilities.createQueryParam(
				requestSpecification, "count", "3"), "get");
	...
	public void read_Tweet_Text() {
		RestUtilities.setEndPoint(EndPoints.STATUSES_USER_TIMELINE);
		Response response = RestUtilities.getResponse(RestUtilities.createQueryParam(
				requestSpecification, "text", "my tweet from eclipse with logging all 2 .then() .log() .all(pretty)"), "get");
